...
* Allow node-level defaulting of namespaced kernel parameters for all pods.
...
* The mechanism will NOT support configuring non-namespaced sysctls in Pods.
...
Yes. To disable the feature, turn off the feature gate, or remove the kubelet config `DefaultPodSysctls`. The new pods will not have default sysctl settins, while existing pods keep unchanged.
